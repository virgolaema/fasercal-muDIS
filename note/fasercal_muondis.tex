...
\midrule
$x>0.2$ (integrated) & $\mathbf{-0.198\pm0.035}$ ($5.7\sigma$) & --- & \\
\bottomrule
\end{tabular}
\end{center}
...
$x>0.2$ ($A=-0.198$, $5.7\sigma$) even though the input PDF has
$c=\bar{c}$ exactly.} It is therefore entirely fragmentation --- the
leading-baryon effect of Sec.~\ref{sec:asym}, which strengthens at large $x$
where the struck quark carries most of the nucleon momentum and the remnant
diquark is readily available to form a $\Lambda_c$. It has the \emph{same sign}
as the predicted intrinsic-charm signal and is comparable in magnitude up to
$x\sim0.5$, so $|S/B|\sim1$ over much of the interesting range rather than the
5--10 claimed before.
...
\begin{enumerate}
  ...
  \item \textbf{Hadronisation model --- now a leading systematic.} The
        $\Lambda_c/\bar{\Lambda}_c=6.8$ leading-baryon effect drives a
        fragmentation asymmetry of $-0.198$ at $x>0.2$ (Sec.~\ref{sec:acx}),
        comparable to the intrinsic-charm signal itself and of the same sign.
        It is Pythia-specific and is not varied here. Any future $A_c(x)$
        measurement is limited by how well this is modelled, not by statistics.

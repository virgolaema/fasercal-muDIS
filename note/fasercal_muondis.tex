...
\quad containing charm & 12\,120 & 81\,900 & $2.86\%$ \\
\quad charm $\to$ semileptonic $\mu$ & 2\,138 & 14\,445 & $17.6\%$ of charm \\
\quad $\mu$ punches out of calorimeter & 1\,745 & 11\,800 & $81.7\%$ \\
\quad reaches spectrometer ($43\%$) & 762 & 5\,167 & $43\%$ \\
\quad identified $+$ linked ($\varepsilon=0.9$) & \textbf{686} & \textbf{4\,650} & $0.16\%$ of DIS \\
\midrule
...
fiducial $z<1150\,$mm ($277\,$kg), $F_{\rm flux}=1.10$, $F_{\rm optics}=2.0$.}
\label{tab:chain}
\end{table}
...
\textbf{3DCal $+$ ECAL $+$ AHCAL} & $\mathbf{2\,644\,}$\textbf{kg} & \textbf{4\,034\,283} & \textbf{115\,300} & \textbf{6\,566} & $\mathbf{0.012}$ \\
\bottomrule
\end{tabular}
\caption{Adding the downstream calorimeters as target volumes. Masses are
fiducial; all rows use $\varepsilon=0.9$, which is \emph{not} appropriate for the
coarser detectors --- see the caveat below.}
\label{tab:volumes}
\end{table}

Using the full stack gives a $9.5\times$ larger target than the 3DCal alone and
improves the statistical reach by $\times3.2$. Material composition is handled
...
\begin{enumerate}
  ...
  \item \textbf{Containment.} The ECAL is only $\sim\!0.7$ interaction lengths
        deep and cannot contain a hadronic shower by itself. It does however sit
        \emph{upstream} of the AHCAL, which provides $\sim\!5\lambda$ behind it,
        so it is given full fiducial acceptance here while the AHCAL --- the last
        calorimeter --- keeps the $48\%$ of CDR Table 8. That asymmetry is a
        modelling choice, not a documented one.
\end{enumerate}
...
$686$ charge-signed, vertex-linked charm muons in the 3DCal alone at $780\fb$;
$2602$ adding the ECAL, $4650$ adding the AHCAL, $6566$ using all three
($\sigma(A_c)$ improving $0.038\to0.012$). The extra volumes are coarser, so
